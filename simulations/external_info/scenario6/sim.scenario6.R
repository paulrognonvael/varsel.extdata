...
setwd(paste0(path,'external_info/scenario6/'))
...
for (n  in values.n){
  t0 <- Sys.time() 
  beta_star0 <- c(bmin0, betas[1:(nb.active(n)/2-1)], rep(0,nb.inactive3(n)))
  beta_star1 <- c(bmin1, betas[(nb.active(n)/2):(nb.active(n)-2)], rep(0,nb.inactive2(n)))
  ...
  write.csv(sim.result.df,paste0("sim.result.scenario6.n",n,".csv"),row.names = FALSE)
