...
leveraging multiple datasets. Integrating multiple sources of data implies handling and possibly exploiting some form of heterogeneity. 
In particular, there is often
...
We refer to this task a structural transfer learning, in contrast to  parameter estimation and prediction tasks more commonly considered in transfer learning.
...
Despite our theoretical focus, we note that $\ell_0$ penalties have also been shown to outperform other penalties in certain practical settings \citep{mazumder2020}. Computational advances have also made $\ell_0$ problems much more tractable: optimization methods can solve them exactly for $p$ in the thousands \citep{bertsimas2020sparse} and Markov Chain Monte Carlo (MCMC) have %with probability going to 1 with 
linear cost in $p$ under sparsity conditions \citep{Yang2016,zhou2022}. There is also a direct connection between $\ell_0$ penalties and Bayesian variable selection based on posterior model probabilities \citep{BIC,EBIC,rossell2022concentration}. Under mild regularity conditions, a choice of $\ell_0$ penalty corresponds to a choice of prior probability for variable inclusion. %the inclusion of variables in the support of $\bbeta^*$. 
...
\paul{we could move this last motivation first actually, and argue $\ell_0$ is the right way to encode prior knowledge on inclusion.}

We consider a variable selection framework where penalties may depend on vector of external information $\bz=(z_1,\ldots,z_p) \in \R^p$ %, where $z_i$ is the external information for variable $i=1,\ldots,p$, 
that partitions the $p$ variables into $b$ blocks, denoted by $B_j \subset \{1,\ldots,p\}$, $j=1,\ldots,b$. For example, in structural transfer learning, $z$ is indicator of whether variables were found to be promising in related source datasets. 
...
The idea is that key characteristics such as the proportion of truly active variables are thought to potentially vary across blocks, hence one allows the $\ell_0$ penalty to differ across blocks. %That is, external information that induces a partition function 
...
and $\beta_{\min,j}^*=\min_{i \in S_j}|\beta^*_i|$. We denote $\mathfrak{B}=\{\bbeta \in \R^p: \text{for all }j \in \{1,\ldots,b\} \text{ and }i \in B_j,\, |\beta_i|\geq  \beta_{\min,j} \text{ if } i\in S_j \text{ and }  \beta_i= 0 \text{ otherwise}\}$ the set of possible data-generating $\bbeta^*$.
...
Finally, we discuss general assumptions used across results (Section~\ref{ssec:genassumptions}).
...
In particular, when $p(\bbeta \mid M)$ is Zellner's unit information prior \citep{zellner1986assessing}, then $c_M$ does not depend on $M$ and \eqref{eq:dfbapon} is exact by replacing $\tilde{\bbeta}^{(M)}= (\bX_M^\top \bX_M + n^{-1} I)^{-1} \bX_M^\top \by$.
...
Now, assume one has external information $\bz=(z_1,\ldots,z_p)$ on the likelihood of variables to be truly associated with $\by$. In a Bayesian spike-and-slab regression, this prior information could be naturally reflected by setting prior inclusion probabilities that are a function of $\bz$, that is replacing $\theta$ in \eqref{eq:modelprior} by $\theta(z_i)$, $i=1,\ldots,p$. In particular, consider the case where $\bz$
induces a partition of the set of variables in blocks $B_1,\ldots,B_b$, %such that the likelihood of being truly associated with $\by$ is thought to potentially vary across the $B_j$'s, 
as may occur in transfer learning. One would then get block-dependent prior inclusion probabilities $\theta(z_i)= \theta_j$ for all $i \in B_j$,
...
This work discusses the properties of $\hat{S}^{ei}$ and \hyperlink{lab:AlgoTL}{Tran-s-ell0} when the number of samples $n$ and  variables $p$ go to infinity. Although not explicitly denoted, $p$, $s$ and $\beta_{\min}^*$ are functions of $n$, and so are $s_j$, $p_j$ and $\beta_{\min,j}^*$. We study asymptotic regimes where

     \hypertarget{lab:A1}{(A1)}\quad $n \,\to \, \infty$, and
     
     \hypertarget{lab:A2}{(A2)}\quad For all $j$, the number of inactive variables $p_j-s_j \,\to \, \infty$.\\[5pt]
Assumption~\hyperlink{lab:A2}{A2} can be relaxed to allow for fixed $p_j-s_j$, but simplifies the exposition. The number of active variables $s_j\geq1$ can either be fixed or $s_j \rightarrow \infty$. No assumption is made about the asymptotic regime linking $n$ and $p$, but the main interest is in $n= o(p)$ settings. We make the following assumption on the set of candidate models $\M$,

  \hypertarget{lab:A2bis}{(A2bis)}\quad $\hat S^{ei}$ is unique, $S\,\in\,\M$ and for any $M \in \M$, $\bX_M$ has full column rank.\\[5pt]
\paul{Added this assumption, I will change the numbering later} Assumption~\hyperlink{lab:A2bis}{A2bis} implies that $\M$ is a well-specified set of candidate models and that one constrains attention to models with full-rank designs. As a consequence we have that, necessarily, $s\leq n$. Non-uniqueness of $\hat{S}^{ei}$ and non-full rank models can be accommodated, though, at the expense of a slightly more involved treatment. \paul{Copied this last sentence from an earlier version. Can we really accommodate $\bX_S$ non full rank?} Finally, we make the following assumption on the external information $\bz$. \\[5pt]
...
In Theorem~\ref{prop:fullrecovreg}, $\delta\in(0,1)$ should be understood as being arbitrarily close to 1. 
After discussing the theorems, we will discuss the tightness of Assumptions \hyperlink{lab:A4}{A4}-\hyperlink{lab:A5}{A5} in relation to related necessary conditions for consistent variable selection.
...
\end{equation}
Moreover, suppose that for some fixed $\gamma^*\in(1/2,1)$, arbitrarily close to $1/2$, and all $j=1,\ldots,b$, the $\kappa_j$ are set at the oracle values such that
...
\end{align}
We bound these maximums %that account for the dependence between models 
through the next two quantities
...
\begin{prop}\label{prop:nectypeItypeII}
 \begin{enumerate}[label=(\roman*)]\tightlist
    ...
    \item Assume \hyperlink{lab:A1}{A1}. If for some $j=1,\ldots,b$, $\kappa_j\to\infty$  and
      $\lim_{n\to \infty}\sqrt{n\bar{\lambda}}\beta_{\min,j}^*-\sqrt{2\kappa_j} < \infty$ then $\lim_{n \to \infty} \, \sup_{\bbeta^*\in\mathfrak{B}} P(\hat{S}^{ei} = S) < 1$.XXXX CHECK PROOF with SUP.
...
\end{eqnarray}
Since for every $j=1,\ldots,b$, $p_j-s_j\leq p-s$, $s_j\leq s$ and $\beta^*_{\min,j}\geq \beta^*_{\min}$, the growth rate of $n$ required in \eqref{eq:rangeblockpenex} is slower than that required in \eqref{eq:rangepenex}. Variable selection consistency can be guaranteed with the informed selector $\hat S^{ei}$ under milder conditions than with the standard selector $\hat S^{sd}$. How much milder depends on how much sparser the global $\bbeta^*$ is compared to the block $\bbeta^*_{j}$'s. In Supplement XXX, we provide examples in different scenarios of informativeness of the block partition.

Similarly, Proposition~\ref{prop:nectypeItypeII} (iii) implies that the growth rate of $n$ necessary for consistency with $\hat{S}^{ei}$ is slower than the growth rate necessary for consistency with $\hat{S}^{sd}$. It follows that in some settings $n$ may not meet the necessary growth rate required for consistency with $\hat{S}^{sd}$ but may meet the sufficient growth rate required for consistency with $\hat{S}^{ei}$. Corollary \ref{cor:selconstimpossiblereg} next describes such settings.

\begin{cor}\label{cor:selconstimpossiblereg}
Assume \hyperlink{lab:A1}{A1}, \hyperlink{lab:A2}{A2}, \hyperlink{lab:A3}{A3}, \hyperlink{lab:A4}{A4}, \hyperlink{lab:A5}{A5} and $\lim_{n\to\infty}(p-s)^{\underline{\lambda}}=\infty$ where $\underline{\lambda}$ as in \eqref{eq:ovpsk} for the case $b=1$. If
...
\end{equation}
Since $\beta_{\min,j}^* \geq \beta_{\min}^*$ and $\max\{p-s,s\}\geq \max\{p_j-s_j,s_j\}\big)$, equation \eqref{eq:fewkn} shows that for every $n$ large enough ${OR}^{ei} \leq {OR}^{sd}$ for any partition in blocks, i.e. the oracle convergence rate in \eqref{eq:oraclerateconvreg} for $\kappa_j^*$ is never worse than for $\kappa^*$.
...
the oracle sets $\kappa^*_j < \kappa^*$, so that smaller signals are detected. If $\beta^*_{\min,j} \gg \beta_{\min}^*$, the oracle sets $\kappa^*_j > \kappa^*$ and the probability of false positives is reduced. See Supplement XXX, for illustrations in four examples contemplating different scenarios of sparsity regimes and informativeness of the blocks.
...
The subset $S_j^{S}(\kappa_j)$ gathers truly non-zero parameters in $S_j$ that are small with respect to the penalty $\kappa_j$, $S_j^{L}(\kappa_j)$ those that are large in that they satisfy Assumption~\hyperlink{lab:A5}{A5}, and $S_j^{I}(\kappa_j)$ those that are neither large nor small. Moreover, let $\T(\kappa)$ be the set of models that contain all large signals in $S^{L}(\kappa)=\cup_{j=1}^b S_j^{L}(\kappa_j)$, and neither truly inactive parameters in $V\setminus S$ nor small signals in $S^S(\kappa)=\cup_{j=1}^b S_j^{S}(\kappa_j)$. That is,
...
\begin{thm}\label{thm:convtoT}
    Assume \hyperlink{lab:A1}{A1}, \hyperlink{lab:A3}{A3}, \hyperlink{lab:A4}{A4}, and \hyperlink{lab:A6}{A6}, for every $j=1,\ldots,b$,  then, \\$\lim_{n \to \infty} \sum_{M \in \M\setminus \T(\kappa)} \, \sup_{\bbeta^*\in\mathfrak{B}}\, \E\left(NC(M)\right)= 0 \mbox{  and  } 
    \lim_{n \to \infty} \, \inf_{\bbeta^*\in\mathfrak{B}}\, P(\hat{S}^{ei} \in \T(\kappa))= 1.$
\end{thm}
Theorem~\ref{thm:convtoT} shows %, assuming only sufficiently large penalties as given in Assumption~\hyperlink{lab:A4}{A4}, 
that the normalized scores, which, recall, can be interpreted as pseudo-posterior model probabilities, concentrate on $\T(\kappa)$. That is, all inactive and small active signals are discarded, large signals are retained and intermediate signals in $S^{I}(\kappa)$ may or may not be retained. 


\section{Data-based externally-informed penalization}\label{sec:informed_l0}

We propose a data analysis method to set block penalties that does not require an oracle and discuss its properties. The proposed adapts to the relevance of the external-information which, as discussed in Section \ref{ssec:regression_gains}, is driven by varying sparsity levels and signal strengths across blocks. Exploiting the connection between informed $\ell_0$ penalties and Bayesian variable selection, we propose an estimator of the proportion of truly active variables in each block inspired by empirical Bayes. Such connection also allows the use of Bayesian computational methods. % that overcome the intractability of $\ell_0$ penalties. 
...
Interpreting $NC(M)$ as the pseudo-posterior probability of model $M$ motivates estimating the number of active variables $s_j$ in block $j$ by $\hat{s}_j= p_j \hat{\theta}_j$. Approximating $\hat{\theta}_j$ by replacing $P(M \mid \by, \btheta)$ by $NC(M)$ gives
...
estimator of $s_j/p_j$ because $S_j=S_j^{L}(\kappa_j)$ and $S_j^{S}(\kappa_j)=\emptyset$ for all $j$. \david{This consequence is not obvious. As stated, the proposition is a result on the expectation, which is different from saying that $\hat{s}_j/p_j$ converges to a constant (it could have non-vanishing variance).} \paul{You're right, I corrected l1 convergence to asymptotically unbiased.} The proposition also shows that, when Assumption~\hyperlink{lab:A5}{A5} is not met, $\hat{s}_j/p_j$ is asymptotically downward biased by at least the proportion of small signals $|S_j^{S}(\kappa_j)|/p_j$, but it is guaranteed to be larger than the proportion of large signals $|S_j^L(\kappa)/p_j$ (i.e., those satisfying \hyperlink{lab:A5}{A5}).
...
This choice of block penalties\david{Do you mean "This choice of $\kappa^\circ$"?} \paul{of $\kappa_j^{A}$, added the hat on $s_j$ next to make it clearer} amounts to setting $\theta_j\approx (p_j-\hat{s}_j+1)^{-1}$ in the BIC approximation \eqref{eq:block_modelprob}.
Such block penalties $\kappa_j^{A}$ \david{Do you mean "Such $\kappa^\circ$"?}\paul{clarified} are sufficiently large for Theorem~\ref{theo:suffcondlinearmodel} to hold, taking $f_j=\ln(n)/2$ in Assumption~\hyperlink{lab:A4}{A4} for all $j$.
...
Our data-based selector $\hat{S}^{A,ei}$ requires similar but slightly stricter betamin condition than the oracle selector using penalties $\kappa_j^*$. Assumption \hyperlink{lab:A9}{A9} ressembles the betamin assumption in \eqref{eq:suffoptim} with $\ln(p_j-s_j)$ replaced by $\ln(p_j-|S^{L}_j(\kappa^{\circ})|)+\frac12\ln(n)$.
...
$\ln(p-|S^{L}(\kappa^{\circ})|)\geq \ln(p_j-|S_j^{L}(\kappa^{\circ})|)$ and $\ln(s)\geq \ln(s_j)$, Assumption \hyperlink{lab:A9}{A9} for $\hat S^{A,ei}$ is milder than \eqref{eq:betaminA} for $\hat S^{A,sd}$, in particular in less sparse blocks where $p_j-|S_j^{L}(\kappa^{\circ})|\ll p-|S^{L}(\kappa^{\circ})|$. Although not shown here, necessary betamin assumptions are also milder for $\hat S^{A,ei}$ than for $\hat S^{A,sd}$.
...
Instead, in the Tran-s-ell0 algorithm below, we treat $\hat{S}^{(k)}$ as external information and perform variable selection in the target data with adaptive informed selector $\hat S^{A,ei}$. In particular, in a first step, for each variable $i=1,\ldots,p$, we define $z_i= I (i\in \cup_{k=1}^K\hat{S}^{(k)})$. %The external information $\bz$ then naturally induces a partition of V, the set of variables in the target dataset, into two blocks: 
...
\end{align*}
where $\xi=\tfrac12(1+\max_j \ln(p_j-s_j)/(\ln(p_j-s_j)+0.5\ln(n)))$.
Assumption~\hyperlink{lab:A10}{A10} is the betamin assumption~\hyperlink{lab:A9}{A9} where the number of active and inactive parameters in blocks are as in \eqref{eq:afihgr} and is sufficient for \hyperlink{lab:AlgoTL}{Tran-s-ell0} to be selection consistent.
...
Observe that in block $B_1$ the more similar $S$ is to the $\hat S^{(k)}$, the smaller is $|(\cup_{k=1}^KS^{(k)}\cap V)\setminus S^L(\kappa^{\circ}))|$ and the milder is \hyperlink{lab:A11}{(A11)} compared to \eqref{eq:betaminA}. \hyperlink{lab:AlgoTL}{Tran-s-ell0} then allows the recovery smaller signals among the variables selected in the source datasets.
...
The adaptive nature of $\hat{S}^{A,ei}$ on which \hyperlink{lab:AlgoTL}{Tran-s-ell0} is based, renders our transfer learning algorithm robust to negative transfer. In the last step, in block $B_1$ that gathers the transferred variables, the penalty is set to $\kappa^{A}_1 = \ln(p_1 -  \hat s_1) + \ln(n)/2$ where $p_1 -  \hat s_1$ is an estimate of $|(\cup_{k=1}^K \hat S^{(k)}\cap V)\setminus S|$. That is, the less informative the transfer, the larger the dissimilarity between $S$ and $\cup_{k=1}^K \hat S^{(k)}$ and the larger the penalty incurred by the transferred variables. 
...
Remarkably, our structural transfer learning algorithm achieves gains under comparatively fewer requirements than transfer learning algorithms for parameter estimation and prediction. Those later class of algorithms tend to either assume the sets of variables are the same between source and target datasets \citep{abba2024} or make strong assumptions on the similarity of design matrices \citep{Li2021, Tian2023} (including an equal number of variables). This is required to avoid negative transfer because parameter estimates are highly sensitive to adjustment sets. Indeed, gains in estimation achievable through transfer learning were  shown to be inherently limited by the dissimilarity between source and target design matrices \citep{liu2025}, an issue also known as the \textit{covariate shift} problem. Our results on structural transfer learning require no such assumption: the variable sets in the source and target datasets may differ largely, even in size.

In addition, many (but not all) transfer learning algorithms for parameter estimation and prediction, e.g \cite{Jiang2016, Li2021}, require access to the source datasets while \hyperlink{lab:AlgoTL}{Tran-s-ell0} only requires sets of selected variables. Our algorithm is then more widely applicable and enjoys stronger privacy and communication efficiency guarantees. 
...
In both sections, we use $\kappa^{\circ}=\tfrac12\ln(n)+\ln(p)$ in Step 1 of our algorithm. 
In Step 1, 
%To search over models, 
we %rely on the connection between $\ell_0$ penalties and Bayesian variable selection and %search over the model space through 
use the MCMC algorithm in function \texttt{bestIC} in R package \texttt{mombf}. Briefly, each visited model is
...
In Step 2, we obtain $\hat S^{A,ei}$ by scoring all the models visited by the MCMC in Step 1. We obtain similarly, for comparison purposes, its uninformed counterparts $\hat S^{A,sd}$ that in Step 2 set a single common penalty. We also compare $\hat S^{A,ei}$ and \hyperlink{lab:AlgoTL}{Tran-s-ell0 algorithm} to the standard $\ell_0$ penalty EBIC \citep{EBIC}, the LASSO \citep{lasso} and SCAD \citep{Fan01SCAD}. For the last two methods, the penalization parameter is chosen by cross-validation. The R code to reproduce our analyses is at 
...
\subsection{Adaptive selector}\label{sec:simulationsss}
...
For each sample size, we generate 100 datasets and report in Figure~\ref{fig:linregprobrec} the empirical probability of correct recovery for $\hat S^{A,ei}$, $\hat S^{A,sd}$, EBIC, LASSO and SCAD. In Examples 1 and 2 where blocks are discriminative, $\hat S^{A,ei}$ outperforms $\hat S^{A,sd}$ and also the EBIC, particularly for small $n$. In the non discriminative block setting of Example 4, $\hat S^{A,ei}$ perform very similarly to $\hat S^{A,sd}$. LASSO and SCAD have consistently lower probability of recovery than the $\ell_0$ methods. The average false discovery rate (FDR) and power in Example 3 reported in Figure~\ref{fig:linregfdrpowmse} shed light on those differences. Compared to the uninformed $\hat S^{A,sd}$, our informed $\hat S^{A,ei}$ has equally low FDR but higher power. LASSO and EBIC have higher power than $\hat S^{A,ei}$ but higher FDR. In terms of mean squared error, $\hat S^{A,ei}$, $\hat S^{A,sd}$ and EBIC perform similarly and better than SCAD and LASSO.
Similar findings are observed in the other examples, and the corresponding plots can found in Supplement XXX. We also report in Supplement XXX the average computation time to obtain $\hat{S}^{A,b}$ as a function of dimension $p$. In all three examples, the average time does not exceed 20 seconds.
...
We first apply the \hyperlink{lab:AlgoTL}{Tran-s-ell0 algorithm} to simulated data. We consider target sample sizes $n$ varying from 20 to 700. For each sample sample size, we generate 100 target datasets with variable values drawn from a multivariate Gaussian with zero means, unit variances, and pairwise correlations $\mbox{cor}(x_{ij}, x_{il})= 0.5$ for $j \neq l$. The target data-generating $\bbeta^*$ satisfies $p-s=3n/2$, $s=3\ln(n)$, and nonzero coefficients are uniformly spaced in $(1/3,1)$. For each target sample size $n$ and each one of the 100 repetitions, we generate two source datasets
with sample size 700 each and dimension equal to that of the target data. Source variable values are drawn from the same multivariate Gaussian distribution as in the target data. The source data-generating parameter is the target $\bbeta^*$  where a proportion $h$ of the nonzero coefficients is set to 0 and an equal number of zero coefficients is set uniformly spaced in $(1/3,1)$. The Hamming distance between the vectors of indicators of active variables in the source and target data is then $2hs$. We consider three scenarios. In Scenario 1, the source and target sets of active variables are the same, $h=0$. In Scenario 2, the source and target sets of active variables are similar but different with $h=0.4$. Scenario 3 is an example of irrelevant transfer where $h=1$. In Scenario 1 and 2, \hyperlink{lab:AlgoTL}{Tran-s-ell0 algorithm} outperforms all the other selectors. In Scenario 3, $\hat S^{A,ei}$  performs similarly to $\hat S^{A,sd}$, better than LASSO and SCAD, slightly worse than EBIC for small $n$ but signficantly better for large $n$.  

Next, we apply \hyperlink{lab:AlgoTL}{Tran-s-ell0 algorithm} to a biological data set. In molecular biology, it is common to use animal models (e.g. mice) to study genomic mechanisms such as gene expression. Findings from such studies often serve as a basis to study human diseases, for example, to assess whether genes that played key roles in the animal model are also important in humans. Specifically, \cite{calon:2012} compared mice where gene TGFB was silenced to mice where it was not, showed that TGFB plays a crucial role in colon cancer progression, and found a list of 172 genes that were associated with TGFB. We use \hyperlink{lab:AlgoTL}{Tran-s-ell0 algorithm} to leverage this set of 172 genes in the selection of genes associated to TGFB in a colon cancer dataset of $n=262$ human samples analyzed in \cite{rossell:2017}. We regress TGFB expression on that of $p=1000$ genes. All variables are standardized to zero mean and unit variance. In this dataset, obtaining $\hat S^{A,ei}$ in Step 2 of \hyperlink{lab:AlgoTL}{Tran-s-ell0 algorithm} took 49 seconds. 
...
Table~\ref{tab:selgenes} reports the genes selected with \hyperlink{lab:AlgoTL}{Tran-s-ell0 algorithm}, $\hat S^{A,sd}$, EBIC, LASSO and SCAD. \hyperlink{lab:AlgoTL}{Tran-s-ell0 algorithm} allows to focus discoveries on the genes that were found to be associated to TGFB in mice. In particular, our algorithm selects more genes from the mouse list, and less genes outside the mouse list than its uninformed counterpart $\hat{S}^{A,sd}$. EBIC selects the same genes as our algorithm in the mouse list, but more genes outside of the mouse list. LASSO and SCAD select a much larger number of genes than the informed and uninformed $\ell_0$ penalties. Given the higher levels of false discovery rates that these methods typically exhibit in correlated design \citep{slope}, those extra discoveries should be considered with caution.

\begin{table*}[ht]
\caption{Selected genes in each block}
\label{tab:selgenes}
\centering
\begin{tabular}{lC{0.1\linewidth}C{0.22\linewidth}C{0.1\linewidth}C{0.22\linewidth}c}
\hline
 & \multicolumn{2}{c}{In mouse list} & \multicolumn{2}{c}{Outside of mouse list} & \\[3pt] \cline{2-5} Method & \multicolumn{1}{C{0.15\linewidth}}{Number of discoveries} & Genes & \multicolumn{1}{C{0.15\linewidth}}{Number of discoveries} & Genes\\ 
\hline
$\hat S^{A,ei}$ & \multicolumn{1}{C{0.1\linewidth}}{4} & ESM1, GAS1, HIC1, CILP & \multicolumn{1}{C{0.1\linewidth}}{0} & - \\
\hline
...
\end{align*}
Since both expressions converge to $1$ as $n$ grows, and the $b$ is fixed by Assumption~\hyperlink{lab:A3}{A3} we get that $$\lim_{n \to \infty} \mathcal{S}=\lim_{n \to \infty} \sum_{M \in \M\setminus \{S\}}\E\left(NC(M)\right)= 0.$$ By Lemma~\ref{lem:L0toL1convergence}, $P(\hat{S}^{ei}\neq S)\leq \,2\, \mathcal{S}$ and then $\lim_{n \to \infty} P(\hat{S}^{ei}= S)=1$.
...
The proof strategy is to show that Assumptions \hyperlink{lab:A1}{A1}, \hyperlink{lab:A3}{A3}, \hyperlink{lab:A6}{A6}, and \hyperlink{lab:A9}{A9} hold to apply Theorem~\ref{theo:informedl0consist} where the block number of active and inactive parameters are given in \eqref{eq:afihgr}. Recall that Theorem~\ref{theo:TLconsist} assumes \hyperlink{lab:A1}{A1} and \hyperlink{lab:A6}{A6} and observe that Assumption~\hyperlink{lab:A3}{A3} holds since the transfer in \hyperlink{lab:AlgoTL}{Tran-s-ell0} induces only two blocks. Hence, it suffices to show that Assumption~\hyperlink{lab:A9}{A9} holds.
