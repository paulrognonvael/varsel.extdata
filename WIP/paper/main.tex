...
We investigate how these conditions can be relaxed by incorporating said external information. A key application that we consider is transfer learning, where variables selected in one or more source datasets are used to guide variable selection in a target dataset.
...
Many contemporary statistical problems involve high-dimensional regression models, where the goal is to identify a small subset of variables associated with a response.
In such settings, inference is often severely limited by the dataset sample size.
At the same time, \color{green}basing inference on a single dataset increasingly feels like a wasted opportunity: \color{black}it is more and more common to have access to external information—such as prior studies, auxiliary datasets, or variable-specific annotations—that can suggest differential relevance across variables. This situation arises, for example, in multimodal data \citep{liang2023multimodal} where different groups of variables may exhibit different levels of sparsity, such as clinical versus genomic markers in biomedical applications.
...
In the context of variable selection, transfer learning uses information about variables identified in source datasets as external input to guide selection in a target regression problem.
We refer to this setting as \emph{structural transfer learning}, to distinguish it from approaches that focus on transferring parameter estimates or predictions.
...
Our other main contribution is proposing a data-based procedure that doesn't require oracle information, and showing that it also enjoys improved properties.
...
We derive the variable selection properties of such informed $\ell_0$ penalties in general and in the context of structural transfer learning, and compare them to standard $\ell_0$ penalties.
...
Section~\ref{sec:TL} applies this procedure to transfer learning and establishes improved selection properties, including robustness to negative transfer in settings where the external data is uninformative regarding what variables are truly active.
...
and $\bbeta^*$ denotes their true values. The set of variable indices is $V=\{1,\ldots,p\}$. For any $A\subseteq V$ and any vector $\bx\in \R^p$, $\bx_A$ denotes the subvector of $\bx$ with entries corresponding to indices in $A$. For any matrix $\bX \in \R^{n\times p}$, $\bX_A$ denotes the submatrix of $\bX$ obtained by selecting the \emph{columns} with indices in $A$. $S=\left\{i\in V:\,\,  \beta^*_i \neq 0 \right\}$ denotes the true support of $\bbeta^*$, its size is $s=|S|$, and hence $p-s$ is the number of truly inactive parameters. The set of candidate models is denoted by $\M$ and is given by subsets $M\subseteq V$. The set $V$ is partitioned into disjoint blocks $B_j \subseteq V$ for $j=1,\ldots,b$, where $b$ is fixed. $S_j=\left\{i\in B_j:\,\,  \beta^*_i \neq 0\right\}$ denotes the set of active parameters in block $B_j$, $s_j=|S_j|$ its size, $p_j-s_j$ the number of inactive parameters in the block. \color{green}We denote by $\beta_{\min,j}^*$ and $\beta_{\min}^*$ lower bounds on the smallest active signal in $S_j$ and $S$ respectively, such that \color{black}
the data-generating $\bbeta^*$ belongs to the set $\mathfrak{B}=\{\bbeta \in \R^p: \text{for all }j \in \{1,\ldots,b\} \text{ and }i \in B_j,\, |\beta_i|\geq  \beta^*_{\min,j} \text{ if } i\in S_j \text{ and }  \beta_i= 0 \text{ otherwise}\}$.\paul{@David, as pointed by Piotr, $\beta_{\min}^*$ and $\beta_{\min,j}^*$ have to be defined as lower bounds on the active signal size, otherwise saying $\bbeta^*\in\mathfrak{B}$ is a circular statement and not meaningful.}
...
We describe externally-informed $\ell_0$ penalization and their connection to Bayesian variable selection (Section~\ref{ssec:intro_blockpen}), and general assumptions used across results (Section~\ref{ssec:genassumptions}).
...
Motivated by this correspondence, we now return to a frequentist perspective. We study in this work the $\ell_0$ selector analog to maximizing $p(M \mid \by, \mathbf{\theta})$ in~\eqref{eq:block_modelprob}. That is, an informed $\ell_0$ selector that penalizes differently the blocks $B_1,\ldots,B_b$,
...
Although not explicitly denoted, $p$, $s$ and $\beta_{\min}^*$ are functions of $n$, and so are $s_j$, $p_j$ and $\beta_{\min,j}^*$. In particular the numbers of inactive and active variables can be either fixed or grow with $n$. We make no assumption about the regime linking $n$ and $p$, but the main interest is in $n= o(p)$ settings. \color{green}For convenience, we assume that in each block $j$, $p_j-s_j\geq1$ and $s_j\geq1$, but our results can easily be adapted to null $p_j-s_j$ and $s_j$.\color{black}
...
We remark that, although the strategy builds upon that in \cite{rossell2022concentration}, there are significant innovations. \color{green}\cite{rossell2022concentration} did not consider block penalties and, when applied to the single penalty case,\color{black} our sufficient conditions to attain consistency are milder and the associated rates are sharper. We also obtain new necessary conditions (Section \ref{ssec:necessary_conditions}) that nearly match the sufficient conditions. 
...
\hypertarget{lab:A4}{(A4)} \quad For each $j$, there exists $f_j\to \infty$ such that $\ln(p_j-s_j)=O(f_j)$ and, 
\begin{equation*}
    \kappa_j \;=\;\ln(p_j-s_j) + f_j
\end{equation*} 
\hypertarget{lab:A5}{(A5)} \quad For each $j$, there exists $g_j\to \infty$ such that,
    \begin{equation*}
        \sqrt{\frac{(1-\gamma) n\rho(\bX)}{6}}{\beta_{\min,j}^*} \,- \,\sqrt{\kappa_j} \;\geq\; \sqrt{\ln(s_j)} + g_j.
    \end{equation*}
...
\paul{I changed A4 to a slightly stronger assumption to ease discussions. In particular $1-\gamma$ is now always bounded away from 0 and we don't have to say in the later discussions that we focus on the penalties $\kappa_j$ such that $\ln(p_j-s_j)=O(f_j)$. If we assume throughout $p_j-s_j\to\infty$, we can write A4 as $\kappa_j \;\geq\;(1+\alpha)\ln(p_j-s_j)$ for all $j$ for some constant $\alpha>0$.}

Assumption~\hyperlink{lab:A4}{A4} sets a lower-bound the block penalties $\kappa_j$, whereas
Assumption~\hyperlink{lab:A5}{A5} imposes a beta-min condition on the block-wise signal strengths $\beta_{\min,j}^*$.
In a simplest orthonormal setting where $\bX^{\top}\bX=n\I_p$, the quantity $\rho(\bX)=1$. More generally, 
$\rho(\bX)\ge 0$ measures the correlation between truly active variables $\bX_S$ and other variable sets $\bX_M$. 
%how well models $M\not\supseteq S$ can be distinguished from $S$; 
For further discussion see, e.g.,~\cite{infotheowainwright}.
Our proofs allow slightly weaker conditions, but we present~\hyperlink{lab:A4}{A4}--\hyperlink{lab:A5}{A5} in this form to facilitate interpretation and comparison with existing results.

We next state our main theorems on strong variable selection consistency of $\hat{S}^{ei}$ and the associated rates. In Theorem~\ref{prop:fullrecovreg}, the constants $\delta\in(0,1)$ and $\gamma^*\in(1/2,1)$ should be taken arbitrarily close to 1 and $1/2$, respectively. After presenting these results, we examine the tightness of Assumptions \hyperlink{lab:A4}{A4}–\hyperlink{lab:A5}{A5} relative to necessary conditions for selection consistency.

\begin{thm}\label{theo:suffcondlinearmodel}

Assume~\hyperlink{lab:A1}{A1}, \hyperlink{lab:A4}{A4} and, \hyperlink{lab:A5}{A5}, and recall that $\mathfrak{B}=\{\bbeta \in \R^p: \text{for all }j \in \{1,\ldots,b\} \text{ and }i \in B_j,\, |\beta_i|\geq  \beta^*_{\min,j} \text{ if } i\in S_j \text{ and }  \beta_i= 0 \text{ otherwise}\}$. Then,
...
Then Assumptions~\hyperlink{lab:A4}{A4} and \hyperlink{lab:A5}{A5} hold, and, if Assumption~\hyperlink{lab:A1}{A1} holds too, for all sufficiently large $n$ and any $\delta \in (0,1)$, we have
...
\begin{prop}\label{prop:nectypeItypeII}
 \begin{enumerate}[label=(\roman*)]\tightlist
    ...
    \item If for some $j\in\{1,\ldots,b\}$,   $\lim_{n\to+\infty}(p_j-s_j)^{\underline{\lambda}_j}=+\infty$ and $\lim_{n\to\infty}\sqrt{n\bar{\lambda}}\beta_{\min,j}^*-\sqrt{2\underline{\lambda}_j\ln(p_j-s_j)} < \infty$, then $\lim_{n \to \infty} \, \sup_{\bbeta^*\in\mathfrak{B}} P(\hat{S}^{ei}=S) < 1$.
\end{enumerate}
\end{prop}

Proposition~\ref{prop:nectypeItypeII} implies that Assumptions \hyperlink{lab:A4}{A4}-\hyperlink{lab:A5}{A5} are fairly tight. By Part (i), the block penalty $\kappa_j$ must grow at least as fast as $\ln(p_j-s_j)$.
...
%$\kappa_j \geq \ln(p_j-s_j)(1+e_j)$,  such that $\lim_{n\to+\infty}e_j \geq 0$, 
nearly matching \hyperlink{lab:A4}{A4}. 
Part (ii) shows that the signal strength $\beta_{\min,j}^*$ must be sufficiently large relative to $\kappa_j$, and closely resembles the requirement that $\sqrt{(1-\gamma) n \rho(\bX) \beta_{\min,j}^*/6} - \sqrt{\kappa_j} \to \infty$ made by \hyperlink{lab:A5}{A5},
up to logarithmic term in $s_j$, and to replacing $\rho(\bX) (1 - \rho)$ by $\bar{\lambda}$, where recall that $1-\gamma \in (0,1/2)$.
...
which again resembles \hyperlink{lab:A5}{A5} in the worst case where $\underline{\lambda}_j=1$.
That is, Assumption~\hyperlink{lab:A5}{A5} and Parts (ii)-(iii) imply the same scalings of $(n,p_j,s_j,\beta_{\min,j}^*)$, up to logarithmic terms.

\subsection{Benefits of externally-informed penalties}\label{ssec:regression_gains}

This section compares the oracle-informed selector $\hat S^{ei}$ with the standard $\ell_0$ selector $\hat S^{sd}$, which possesses the best-known properties %performance among uninformed procedures 
in terms of variable selection consistency \citep{gao2025optimalityl0}.
We highlight two types of improvements in $\hat S^{ei}$: %made possible by externally-informed penalties: 
...
By Theorem~\ref{prop:fullrecovreg}, Assumptions~\hyperlink{lab:A4}{A4}–\hyperlink{lab:A5}{A5} are sufficient for consistent support recovery, both for $\hat S^{ei}$ and $\hat S^{sd}$ (as the particular case where $b=1$).
To compare $\hat S^{ei}$ and $\hat S^{sd}$, we plug in the smallest penalties $\kappa_j$  allowed by \hyperlink{lab:A4}{A4}–\hyperlink{lab:A5}{A5} and inspect the implied scaling of the sample size $n$.

% For the informed selector $\hat{S}^{ei}$ with block penalties set to $\kappa_j=\ln(p_j-s_j)(1+e)$ for some arbitrarily small sequence $e$ such that $\lim_{n\to\infty}e >0$, a sufficient condition on $n$ to have consistency is, for some arbitrarily small sequence $g$ such that $\lim_{n\to\infty}g = \infty$ and up to constants and factors $(1-\gamma_j)$ bounded in $(0,1/2)$,
...
The magnitude of this improvement depends on how strongly $(\beta_{\min,j}^*, s_j, p_j-s_j)$ vary across blocks, whether the blocks truly capture variables with different signal strengths or sparsity levels. %discriminative of inactive variables the external information $\bz$ and the induced partition are.
To illustrate, \color{green}we discuss the requirement on signals strength implied by \eqref{eq:rangeblockpenex} and \eqref{eq:rangepenex}.\color{black} Consider a $b=2$ blocks example where the number of inactive variables satisfies \color{green}$p-s=n^2$\color{black}.
Suppose block~1 contains \color{green}$p_1-s_1=n^{1/2}$ \color{black} inactive variables, while block~2 contains \color{green}$p_2-s_2=n^{2}-n^{1/2}$\color{black}.
Suppose also that $s_1=s_2=O(1)$ and that the design is orthonormal, so that $\rho(\bX)=1$. \color{green}For the standard selector $\hat S^{sd}$, consistency requires $\beta^*_{\min}\gtrsim\sqrt{2\ln n / n}$.
In contrast, for the informed selector $\hat S^{ei}$, consistency requires in block~2, $\beta^*_{\min,1}\gtrsim\sqrt{\ln n / (2n)}$, but in block~1 the requirement is only $\beta^*_{\min,2}\gtrsim\sqrt{\ln n / n}$. That is, $\hat S^{ei}$ can recover signals twice smaller than $\hat S^{sd}$ in block 1. \color{black}Additional scenarios illustrating how gains depend on block sparsity and signal heterogeneity are provided in Supplement~XXX.\paul{We discuss first the implied scaling of $n$, then that of $\beta_{\min}$ and $\beta_{\min,j}$. David suggested we change that. We could discuss scalings of $\beta_{\min}$ and $\beta_{\min,j}$, first and then. However, I feel like the difference in scalings of $n$ is easier to understand because you have $n$ on the LHS in both \eqref{eq:rangeblockpenex} and \eqref{eq:rangepenex}. Discussing scalings of $n$ when we discuss the impact of varying block sparsity is hard because $p$ is expressed as a function on $n$ (usual way to do it).}
...
Corollary~\ref{cor:selconstimpossiblereg} assumes a high-dimensional setting, where $\underline{\lambda} \ln(p-s)$ diverges, %the global \textit{effective} number of inactive variables $(p-s)^{\underline{\lambda}}$ diverges, 
as in Proposition~\ref{prop:nectypeItypeII} (iii). 

\begin{cor}\label{cor:selconstimpossiblereg}
Assume \hyperlink{lab:A1}{A1},  \hyperlink{lab:A4}{A4}, \hyperlink{lab:A5}{A5} and $\lim_{n\to\infty}(p-s)^{\underline{\lambda}}=\infty$ for $\underline{\lambda}$ as in~\eqref{eq:ovpsk} for the $b=1$ block case. If
...
\end{cor}
%It shows that incorporating external information can make consistent selection possible in regimes where $\hat S^{sd}$ cannot be consistent. 
Supplement XXX provides an illustration in Example 2.

We next compare the probabilities of correct selection when both procedures are consistent. Assume that block penalties are set to their oracle values $\kappa_j^*$ and that the standard penalty is set to $\kappa^*$. Let $OR^{ei}$ and $OR^{sd}$ denote the corresponding oracle convergence rates in \eqref{eq:oraclerateconvreg}. Up to constant factors, their ratio satisfies
...
Moreover, if $\beta^*_{\min,j}>\beta^*_{\min}$ for some block $j$, then $OR^{ei}<OR^{sd}$, implying faster convergence. \david{Is this really true? If $\beta^*_{\min,1}>\beta^*_{\min}$ in block $j=1$ but $\beta^*_{\min,2}=\beta^*_{\min}$, and similarly $(p-s) = (p_1-s_1)/2= (p_2-s_2)/2$ then \eqref{eq:fewkn} doesn't vanish as $n \to \infty$. That is, for the sum across $b$ to vanish we need that each term $j$ vanishes, and for that we need that either $\beta^*_{\min,1}>\beta^*_{\min}$ or $(p-s) / (p_j - s_j) \to \infty$. If my reasoning is correct, let's update the discussion above and below.}\paul{We don't claim the ratio vanishes, only that $OR^{ei}$ is smaller. In the case that you describe with $p_j-s_j>s_j$, I think the ratio is $2^{-\delta/2}<1$ with $\delta$ close to 1. We could add: "Further, if for every $j$, $\beta^*_{\min,j}>\beta^*_{\min}$ or $(p-s) / (p_j - s_j) \to \infty$, $OR^{ei}=o(OR^{sd})$"}

These gains are driven by the difference in sparsity across blocks. Consider a sparse setting where for all $j$, $\max\{p_j-s_j,s_j\}=p_j-s_j$. There are then two sources of improvement in convergence rates: in any block $j$, a smaller number of truly zero parameters, that is $p_j-s_j< p-s$, or a larger signal, $\beta^*_{\min,j} > \beta_{\min}^*$. In blocks where $\beta^*_{\min,j} \approx \beta_{\min}^*$, improvement comes mainly from having a smaller number of truly zero means. In those blocks,
the oracle sets $\kappa^*_j < \kappa^*$, so that smaller signals are detected. If $\beta^*_{\min,j}$ is much greater than $\beta_{\min}^*$, the oracle sets $\kappa^*_j > \kappa^*$ and the probability of false positives is reduced. Further illustrations are provided in Supplement~XXX.

% First, blocks with fewer inactive variables satisfy $p_j-s_j<p-s$, which reduces the effective multiple-testing burden.
% Second, blocks with larger minimal signal strength $\beta^*_{\min,j}$ allow more aggressive penalization, reducing false positives. Depending on the block structure, either effect may dominate. Further illustrations are provided in Supplement~XXX.}

\subsection{Generalized convergence}\label{ssec:nobetaminhyp}

We now discuss a convergence result for $\hat S^{ei}$ that is a key ingredient for the data-driven procedures in Section~\ref{sec:informed_l0}. This result generalizes Theorem~\ref{theo:suffcondlinearmodel} by dropping the beta-min Assumption~\hyperlink{lab:A5}{A5}. Instead, active signals are separated into three categories---\emph{small}, \emph{intermediate}, and \emph{large}---relative to the penalties $\kappa_j$. We define next these three categories.
%With sufficiently large penalties (as in Assumption~\hyperlink{lab:A4}{A4}), the procedure discards all inactive variables and also discards the small active signals.
%At the same time, it retains all large signals (in a sense made precise below), while intermediate signals may or may not be selected.

For fixed block penalties $\kappa_1,\ldots,\kappa_b$, and for $\gamma$ \color{green} and the $g_j$ \color{black} as in Assumption~\hyperlink{lab:A5}{A5}, define
\begin{eqnarray}\label{eq:defSes}
    S_j^{L}(\kappa_j)
    &:=&
    \Big\{ i \in S_j \,\Big|\, \sqrt{\frac{(1-\gamma) n\rho(\bX)}{6}}\,|\beta_i^*|-\sqrt{\kappa_j}
    \;\geq\;\color{green}\sqrt{\ln(s_j)} + g_j\color{black}\Big\},\label{eq:defslsi}\\
    S_j^{S}(\kappa_j)
    &:=&
...
The set $S_j^{L}(\kappa_j)$ contains the large parameters, in that they meet the beta-min Assumption~\hyperlink{lab:A5}{A5}. The set $S_j^{S}(\kappa_j)$ contains small signals relative to the penalty $\kappa_j$. The remaining active variables form the intermediate set $S_j^{I}(\kappa_j)$.
...
Theorem \ref{thm:convtoT} shows that, under Assumption~\hyperlink{lab:A4}{A4}, the normalized scores $NC(M)$ in \eqref{eq:CM} concentrate on models that include all large signals, exclude all inactive variables and all small signals, and differ only in which intermediate variables they include: %That is, they concentrate on
...
Assumption~\hyperlink{lab:A6}{A6} is mild and can be relaxed:  
...
Further, if $|S_j^{S}(\kappa_j)|/ (p_j-s_j) \to \infty$ then Theorem \ref{thm:convtoT} still holds, replacing, $p_j-s_j$ by $|S_j^{S}(\kappa_j)|$ in \hyperlink{lab:A4}{A4}.
\begin{thm}\label{thm:convtoT}
    Assume \hyperlink{lab:A1}{A1}, \hyperlink{lab:A4}{A4}, and \hyperlink{lab:A6}{A6}, for every $j=1,\ldots,b$,  then, \\$\lim_{n \to \infty} \sum_{M \in \M\setminus \T(\bkappa)} \, \sup_{\bbeta^*\in\mathfrak{B}}\, \E\left(NC(M)\right)= 0 \mbox{  and  } 
    \lim_{n \to \infty} \, \inf_{\bbeta^*\in\mathfrak{B}}\, P(\hat{S}^{ei} \in \T(\bkappa))= 1.$
\end{thm}
%Theorem~\ref{thm:convtoT} provides a concentration result for informed or standard $\ell_0$ penalties that does not require a beta-min assumption and holds for any $\beta_{\min,j}$, $j=1,\ldots,b$. 
...
The goal is to adapt to how informative the external information is, that is, to differences in sparsity across blocks (Section~\ref{ssec:regression_gains}). Our construction uses the Bayesian interpretation of $\ell_0$ penalties via the BIC approximation in~\eqref{eq:block_modelprob}.
...
Recall that under the BIC approximation in~\eqref{eq:block_modelprob}, choosing $\kappa_j$ is equivalent to choosing a block-wise prior inclusion probability %$\theta_j$ via
$\theta_j=(\exp(\kappa_j-\ln(n)/2)+1)^{-1}$,
which is %The prior probability $\theta_j$ represents 
...
This choice of penalties amounts to setting $\theta_j= (p_j-\hat{s}_j+1)^{-1}$ in the BIC approximation~\eqref{eq:block_modelprob} and the resulting block penalties $\kappa_j^{A}$ 
are sufficiently large for Theorem~\ref{theo:suffcondlinearmodel} to hold. \color{green} We briefly discuss alternative choices of block penalties at the end of the section.\color{black}
...
%Step~1 uses a common penalty $\kappa^\circ$, which under the BIC approximation corresponds to setting equal prior inclusion probabilities across blocks. It 
Step 1 can be carried out with any Bayesian computational method that returns variable posterior inclusion probabilities $P(\beta_i \neq 0 \mid \by, \btheta)$. 
...
\hypertarget{lab:A9}{(A9)}\quad For each block $j$, there exists $d_j\to\infty$, such that
\begin{equation*}
...
Under Assumptions~\hyperlink{lab:A1}{A1}, \hyperlink{lab:A6}{A6}, and \hyperlink{lab:A9}{A9}, we show that, with probability going to 1, $\kappa^{A}_j$ and $\beta_{\min,j}^*$ satisfy Assumptions~\hyperlink{lab:A4}{A4}--\hyperlink{lab:A5}{A5} and that Theorem~\ref{theo:suffcondlinearmodel} applies. The next result on the consistency of $\hat{S}^{A,ei}$ then follows.
...
\color{green}A natural alternative to the $\kappa^{A}_j$ for block penalties in Step 2 of Algorithm 1 is $\kappa^{EB}_j=\ln(p/\hat{s}_j-1)$. This choice amounts to setting prior inclusion to the empirical Bayes estimate $\theta_j=\hat{s}_j/p_j$ in the BIC approximation \eqref{eq:block_modelprob}. Consistency of the resulting procedure can be shown at the cost of making an additional assumption bounding the block number of active signals. Gains in conditions for consistency in block $j$, with respect to using a single common penalty $\kappa^{EB}=\ln(p/\hat{s}-1)$, then require $s_j/p_j>s/p$ is positive. This is reasonable for an informative block partition but not guaranteed for any partition.\color{black}

\section{Structural transfer learning}\label{sec:TL}

This section studies a structural transfer learning method built on the adaptive informed selector $\hat S^{A,ei}$.
We assume that for each source dataset $k=1,\ldots,K$ we are given a set of selected variables $\hat{S}^{(k)}$, and we want to leverage these sets to improve variable selection in a target dataset.
...
In a worst case, the set of truly active variables in the source and target may not overlap at all.
...
follow from Theorem~\ref{theo:informedl0consist}, applied to the blocks defined using the $\hat{S}^{(k)}$'s. Let
...
\hypertarget{lab:A10}{(A10)}\quad There exist $d_1,d_2\to \infty$ such that,
\begin{align*}
...
Assumption~\hyperlink{lab:A10}{A10} is Assumption~\hyperlink{lab:A9}{A9}, plugging in the number of active ($s_j$) and inactive ($p_j-s_j$) parameters in~\eqref{eq:afihgr}.

\begin{cor}\label{cor:TLconsist}
    Assume \hyperlink{lab:A1}{A1}, \hyperlink{lab:A6}{A6}, and \hyperlink{lab:A10}{A10}, then $\lim_{n \to \infty} \,\inf_{\bbeta^*\in\mathfrak{B}}P(\hat{S}^{tr} = S) = 1$.
\end{cor}
...
%The gains depend on the similarity between the true set of active variables in the target dataset and the sets of variables selected in the source datasets. 
%In block $B_1$, the closer $S$ is to the transferred set $\cup_{k=1}^K\hat S^{(k)}$, the smaller the number of inactive variables and$|(\cup_{k=1}^K\hat S^{(k)})\setminus S^L(\kappa^\circ)|$, and 
the milder \hyperlink{lab:A10}{A10} becomes relative to~\eqref{eq:betaminA}.
...
\hline
Scenario & $p-s$ & $s$ &\color{green} $s_1/(p_1-s_1)$ &\color{green} $s_2/(p_2-s_2)$ & $\beta_{\min,1}^*$ & $\beta_{\min,2}^*=\beta_{\min}^*$ \\
\hline
$1$    & $3n/2$ & $3\ln(n)$ & $3\ln(n)/(3n-2\sqrt{n})$ & $3\ln(n)/(2\sqrt{n})$  & 0.5 & 0.33\\
$2$    & $n$ & $3\ln(n)$ & $3\ln(n)/(2(n-\ln(n)))$ & $3/2$   & 0.5 & 0.33 \\
$3$    & $n$ & $3\ln(n)$ & $3\ln(n)/n$ & $3\ln(n)/n$  & 0.5 & 0.5\\
\hline
\end{tabular}
\end{center}
\end{table*}
\paul{David asked to report the exact $s_j/p_j$ in the table. I designed the simulation with $p_j-s_j$ and $s_j$ as function of $n$, so the exact $s_j/p_j$ are quite complex. I suggest reporting $s_j/(p_j-s_j)$ instead.}

Table~\ref{tab:examples_bis} summarizes our three simulation scenarios, which differ in the sparsity and the informativeness of the blocks. In Scenarios 1 and 2 the blocks are informative: block $B_1$ is sparser than $B_2$. In Scenario 1, the blocks are moderately informative, in that $(s_2/p_2)/(s_1/p_1)= O(\sqrt{n})$. In Scenario 2, the blocks are highly informative: $\bbeta^*$ is sparse overall, but it is non-sparse within block 2 ($s_2 > p_2-s_2)$. In Scenario 3 the blocks are non-informative: each %is a non-discriminative average random guess where each block 
...
\begin{tabular}{M{49mm}M{49mm}M{49mm}}
        Sc.1 Prob. recovery& Sc.2 Prob. recovery & Sc.3 Prob. recovery\\
         \includegraphics{images/p.probrecov.ex1.pdf} & \includegraphics{images/p.probrecov.ex3.pdf} & \includegraphics{images/p.probrecov.ex4.pdf}
\end{tabular}
...
%performs similarly to the EBIC for small $n$. For large $n$, $\hat S^{A,ei}$ outperforms the EBIC and performs similarly to $\hat S^{A,sd}$. 
In Scenario 3, where blocks are non-discriminative, $\hat S^{A,ei}$ performs similarly to $\hat S^{A,sd}$. LASSO and SCAD have significantly lower probability of recovery than the $\ell_0$ methods. The average false discovery rate (FDR) and power in Scenario 2, reported in Figure~\ref{fig:linregfdrpowmse}, shed light on these differences. Compared to the uninformed $\hat S^{A,sd}$, our informed $\hat S^{A,ei}$ has equally low FDR but higher power. LASSO and EBIC have higher power than $\hat S^{A,ei}$ but higher FDR. In terms of mean squared error, $\hat S^{A,ei}$, $\hat S^{A,sd}$ and EBIC perform similarly, and better than SCAD and LASSO.
Similar findings are observed in Settings 1 and 3, and the corresponding plots can be found in XXX in Supplement XXX. \david{Instead, say "see Figure SXX".} We also report in XXX in Supplement XXX \david{Specify the table/fig number} the average computation time to obtain $\hat{S}^{A,sd}$ as a function of dimension $p$. In all three settings, the average time was below 20 seconds.
...
\begin{tabular}{M{52mm}M{52mm}M{52mm}}
         ...
         \includegraphics{images/p.TL.probrecov.ex1.pdf} &          \includegraphics{images/p.TL.probrecov.ex3.pdf} &      \includegraphics{images/p.TL.probrecov.ex5.pdf} 
\end{tabular}
...
We apply next the \hyperlink{lab:AlgoTL}{Tran-s-ell0} algorithm to simulated data. We consider target dataset sample sizes $n \in [20,700]$ and, for each sample size, obtain 100 simulations. %generate 100 sets of target and source datasets. 
...
We consider three scenarios. In Scenario 1, the truly active variables in the source and target sets are the same, that is $h=0$. In Scenario 2, there is a moderate overlap, with $h=0.4$. Scenario 3 is an example of completely irrelevant transfer where $h=1$. 
Variable values are drawn from a Gaussian with zero means, unit variances in both the source and target datasets \color{green}but with different correlation structures. For any two variables $j \neq l$, the pairwise correlation is $\mbox{cor}(x_{ij}, x_{il})= 0.5$ in the target datasets, while it is $\mbox{cor}(x_{ij}, x_{il})= 0.5^{|i-j|}$ in the source datasets.\color{black} 
...
Table~\ref{tab:selgenes} reports the genes selected with Tran-s-ell0, $\hat S^{A,sd}$, EBIC, LASSO and SCAD. Compared to the uninformed selector $\hat{S}^{A,sd}$, Tran-s-ell0 focuses discoveries on the genes that were found to be associated with TGFB in mice: it selects more genes from the mouse list, and less genes from outside the list. LASSO and SCAD select a much larger number of genes than Tran-s-ell0, although recall that in our simulations in Section~\ref{sec:simulationsss} these methods exhibited high  false discovery rates (see also \cite{slope}). The extra discoveries of LASSO and SCAD should hence be considered with caution. Finally, the EBIC selects the same genes from the mouse list as Tran-s-ell0 but also three more genes from outside the list. In our simulations, EBIC had a slightly larger FDR than our informed selector but also a slightly larger power, for small sample sizes. Then, to assess the relevance of the extra discoveries of EBIC and the quality of the models selected by EBIC and Tran-s-ell0, we report in Table~\ref{tab:cvmse.pseudoprob} the associated cross-validated mean squared error (CV-MSE) in estimation and some pseudo-posterior inclusion probabilities, that is, $\pi(\beta_i)=\sum_{M\in\M} NC(M)I(i\in M)$. We observe that, compared to the EBIC, Tran-s-ell0 leads to lower mean squared error and to higher pseudo-posterior inclusion probabilities for the genes in the mouse list. %We also found that the model selected by Tran-s-ell0 was unchanged when, in Step~(ii), we replaced $\hat S^{A,ei}$ by an alternative informed selector with higher power than EBIC in our simulations. \david{This last sentence is just gonna confuse the reader. I think that what you already discussed should make a convincing case.}Details are provided in Supplement~XXX.


\section{Discussion}
We studied how externally informed penalization can improve variable selection in high-dimensional regression when external information suggests that some variables are more likely than others to be relevant to the response. 
...
We then proposed a data-based procedure that learns block penalties from the target data and retains the main qualitative benefits of the oracle selector. 
...
\begin{description}
\item[S1 Examples of benefits of $\hat S^{ei}$:]
...
\item[S2 Complementary material on numerical illustrations:]
...
$2$    & $\exp(n/20)$ & $\exp(n/20)-n^2$ & $n^2$  & $3\ln(n)/2$  & $3\ln(n)/2$   \\
...
\end{equation}
with $g(\gamma^*)=\sqrt{(-1 + 2 \gamma^*)(1-\gamma^*)}/(1 + \sqrt{2 - 2 \gamma^*})$ then $\kappa^*_j$ defined in~\eqref{eq:oraclepenreg} satisfies Assumptions~\hyperlink{lab:A4}{A4} and \hyperlink{lab:A5}{A5}. Indeed, under~\eqref{betamincond:minimalreg}, for every $j$, there exists a sequence $x_j$ such that $\lim_{n\to\infty} x_j\geq1$ and $g(\gamma^*)\sqrt{\frac{n}{6}}\beta^*_{\min,j}=x_j\big(\sqrt{\ln(p_j-s_j)} + \sqrt{\ln(s_j)}\big)$. Let $v_j= x_j \sqrt{(1-\gamma^*)}g(\gamma^*)^{-1}$, then $\sqrt{\frac{(1-\gamma^*)n}{6}}\beta^*_{\min,j}=v_j\big(\sqrt{\ln(p_j-s_j)} + \sqrt{\ln(s_j)}\big)$. Denote $a=\sqrt{\ln(p_j-s_j)}$ and $b=\sqrt{\ln(s_j)}$.
We have
$$
\sqrt{\kappa^*_j} \;=\;\frac{v_j}{2} (a+b)+\frac{(a^2-b^2)}{2v_j(a+b)}\;=\;\frac{v_j}{2} (a+b)+\frac{a-b}{2v_j}\;=\;\frac{v_j^2+1}{2v_j}a+\frac{v_j^2-1}{2v_j}b.
$$
Since $\sqrt{(1-\gamma^*)}g(\gamma^*)^{-1}>1$ for $\gamma^*\in(1/2,1)$ and $\lim_{n\to\infty} x_j\geq1$, for $n$ large enough, $v_j>1$. We also have $b\geq 0$. It follows that $b(v_j^2-1)/2v_j \geq 0$ for $n$ large enough and
...
\end{equation}
which implies Assumption~\hyperlink{lab:A4}{A4} since $v_j>1$ for $n$ sufficiently. We also have
\begin{equation}\label{eq:oabpx}
\frac{\sqrt{(1-\gamma^*) n\rho(\bX)}}{6}{\beta_{\min,j}^*}-\sqrt{\kappa^*_j} = \frac{v_j^2-1}{2v_j}a+\frac{v_j^2+1}{2v_j}b \geq \Big(1+\frac{(v_j-1)^2}{2v_j}\Big)\sqrt{\ln(s_j)}.
\end{equation}
Let $\gamma = \tfrac12(1+\max_j\ln(p_j-s_j)/\kappa^*_j)$ as defined in Assumption \hyperlink{lab:A5}{A5}. By~\eqref{eq:ojdv}, $\gamma = \tfrac12(1+\max_j(1+(v_j-1)^2/(2v_j))^{-2}$. Since $\lim_{n\to\infty} x_j\geq 1$, for sufficiently large $n$, $v_j\leq \sqrt{(1-\gamma^*)}g(\gamma^*)^{-1}$. Simple algebra shows the latter inequality implies $\gamma=\tfrac12(1+\max_j\ln(p_j-s_j)/\kappa^*_j) < \gamma^*$ and $1-\gamma > 1-\gamma^*$. Then, by~\eqref{eq:oabpx}, 
\begin{equation*}
\frac{\sqrt{(1-\gamma) n\rho(\bX)}}{6}{\beta_{\min,j}^*}-\sqrt{\kappa^*_j} \geq \Big(1+\frac{(v_j-1)^2}{2v_j}\Big)\sqrt{\ln(s_j)}.
\end{equation*}
for every sufficiently large $n$. Since $\lim_{n\to\infty} x_j\geq1$ and $\sqrt{(1-\gamma^*)}g(\gamma^*)^{-1}>1$ for $\gamma^*\in(1/2,1)$, $v_j>1$ for $n$ sufficiently large and  Assumption~\hyperlink{lab:A5}{A5} holds.
...
\textbf{Part (ii).}\,\,
Consider any model $M\in\U_j$, that is under-fitting $S$ by a variable in block $j$ associated with $\beta_{\min,j}^*$. The event $\hat{S}^{ei}=S$ (correct recovery of $S$) requires the event $\frac{NC(M)}{NC(S)}<1$ where $NC$ is the normalized criterion in~\eqref{eq:CM}. That is $S$ is preferred to model $M$. 
\begin{align*}
P(\hat{S}^{ei}=S) 
...
we choose $T^M\in \T(\bkappa)$ such that %for every $M\not\in \T(\bkappa)$, 
...
get an upper-bound $\sum_{M \in \M\setminus \T(\bkappa)} \E\left(NC(M)\right)$ that vanishes (as $n$ grows) under the assumptions of Theorem~\ref{thm:convtoT}. Then Lemma~\ref{lem:L0toL1convergencegen} immediately implies that $P(\hat{S}^{ei}\not \in \T(\bkappa))$ also vanishes.

For the first step of the proof, recall that the set $\T(\bkappa)$ contains all the models that are the union of $S^L(\kappa)$ (large signals) and some subset of $S^{I}(\bkappa)$ (intermediate signals). For any $M \not\in\T(\bkappa)$, take the unique $T^M\in\T(\bkappa)$ such that $T^M = \big[M\cap S^{I}(\bkappa)\big]\cup S^L(\kappa) $, which implies $M\cap S^{I}(\bkappa)=T^M\cap S^{I}(\bkappa)$. 
...
Here $M\setminus T^M$ is not necessarily a subset of $S^C$, hence $\mu_{Q_T^M T^M} \geq 0$. Note that, since $T^M\in\T(\bkappa)$, we have that $S^L(\kappa)\subseteq T^M$. Moreover we have $M\cap S^{I}(\bkappa)=T^M\cap S^{I}(\bkappa)$. It follows that $M\setminus T^M\subseteq (S^{I}(\bkappa) \cup S^L(\kappa))^C$, that is the elements of $M\setminus T^M$ are either inactive or belong to $S^S(\kappa)$. If $M\cap S^{S}(\bkappa)=\emptyset$ then $M\setminus T^M\subseteq S^C$ and we immediately get $\mu_{Q_{T^M} T^M}=o(A_{T^M})$ because $\bbeta^*_{M \setminus T^M}=\mu_{Q_{T^M} T^M}=0$. Assume now that $M\cap S^{S}(\bkappa)\neq\emptyset$. Using that the rightmost component in~\eqref{eq:nonoverfittingcond2} is nonnegative and Lemma~\ref{lem:upperboundnoncentralgen}, we have
...
Moreover, since  $M\setminus T^M\subseteq (S^{I}(\bkappa) \cup S^L(\kappa))^C$ as discussed earlier, we have, for all $j=1,\ldots,b$, $(S_j\cap M_j )\setminus T^M_j \subseteq S_j^{S}(\kappa_j)$. It follows $\max_{i\in(S_j\cap M_j )\setminus T^M_j}{\beta_{i}^*}^2 \leq \max_{i\in S_j^S}{\beta_{i}^*}^2$ and
...
\end{align*}
where last inequality follows from $\Big(1+\sum_{l\neq j}\frac{|(S_l\cap M_l)\setminus T^M_l|\gamma\kappa_l}{|(S_j\cap M_j )\setminus T^M_j|\gamma\kappa_j}\Big)\geq 1$ for all $j$ and from the identity $\sum_{j=1,|(S_j\cap M_j )\setminus T^M_j|\neq 0}^b|(S_j\cap M_j )\setminus T^M_j|\gamma\kappa_j=\sum_{j=1}^b \big|(S\cap M_j)\setminus T^M_j\big|\gamma\kappa_j$. Then, %when $M\cap S^{S}(\bkappa)\neq\emptyset$, 
by~\eqref{eq:onbfsa},
$$\frac{\mu_{Q_{T^M} T^M}}{ A_{T^M}} \;\leq\; \bar{r} \;= \;\sum_{j=1}^b\frac{n\bar{\lambda}\max_{i\in S_j^{S}(\kappa_j)}{\beta_i^*}^2}{\gamma\kappa_j}.$$
By definition of $S_j^{S}(\kappa_j)$, $n\bar{\lambda}\max_{i\in S_j^{S}(\kappa_j)}{\beta_i^*}^2=o(\kappa_j)$ for all $j$ and $\mu_{Q_{T^M} T^M}=o(A_{T^M})$ when $M\cap S^{S}(\bkappa)\neq\emptyset$ too.
We can now apply Lemma~\ref{lem:boundexpnc} and get that for every $M\in \mathcal M\setminus \T(\bkappa)$, for any $\psi\in(0,1)$ and every $n$ large enough, $\E(NC(M))\;\leq\;e^{-\psi A_{T^M}}$.
...
For the final step of the proof, denote $\mathcal{S}= \sum_{M \in \M\setminus \T(\bkappa)} \E\left(NC(M)\right)$ for convenience. By~\eqref{eq:fyv2} we have
\begin{align*}
    \mathcal{S} &\leq \sum_{M \in \M\setminus \T(\bkappa)}e^{- \sum_{j=1}^b |M_j\setminus T^M_j|\big(\ln(p_j-s_j)+\delta \tfrac{f_j}{2}\big)-\sum_{j=1}^b |T^M_j\setminus M_j|\big(\ln(s_j)+\delta \tfrac{g'_j}{2}\big)}.
\end{align*}
We split the sum in the right-hand side above into sums over models $M$ %$M\in \M\setminus \T(\bkappa)$ 
...
\begin{align}\label{eq:gepihw}
    \mathcal{S} &\leq \sum_{T \in \T(\bkappa)}\sum_{M \in \M(T)}e^{- \sum_{j=1}^b |M_j\setminus T_j|\big(\ln(p_j-s_j)+\delta \tfrac{f_j}{2}\big)-\sum_{j=1}^b |T_j\setminus M_j|\big(\ln(s_j)+\delta \tfrac{g'_j}{2}\big)}.
\end{align}
...
\begin{align}\label{eq:gepihw2}
    \mathcal{S} &\leq \sum_{T \in \T(\bkappa)}\big(\mathcal{S}(T)-1\big).
\end{align}
...
Each of the summand vanishes as $n\to\infty$. Moreover, by assumption $|S^I(\kappa)|=O(1)$ and then $|\T(\bkappa)|=2^{|S^I(\kappa)|}=O(1)$. We thus have $\lim_{n\to\infty}\mathcal{S}=\lim_{n\to\infty}\sum_{M \in \M\setminus \T(\bkappa)} \E\left(NC(M)\right)=0$.

Further, by Lemma~\ref{lem:L0toL1convergencegen}, 
$P(\hat{S}^{ei} \not\in \T(\bkappa) ) \,\,\leq \,\, (|\T(\bkappa)|+1) \mathcal{S} = (2^{|S^{I}(\bkappa)|}+1) \mathcal{S}$. Since $\mathcal{S}$ vanishes and $|S^{I}(\bkappa)|=O(1)$, $\lim_{n\to\infty}P(\hat{S}^{ei} \in \T(\bkappa) )=1$ as claimed.
...
\end{eqnarray}
where the last equality follows from $I(i\in M_j)=1$ for all $i\in S^L(\kappa)$ when $M\in\T(\bkappa)$. The rightmost term above and $C_j$ are nonnegative, then by the linearity of the expectation
...
We now prove the upper bound. Recall that $\T(\bkappa)$ by definition includes models that have no small signals, i.e. all parameters are in $S^L(\kappa) \cup S^I(\kappa)$. That is,
%Since 
...
\end{align*} 
Then, with probability going to 1, Assumption~\hyperlink{lab:A11}{A11} implies Assumption~\hyperlink{lab:A9}{A9} holds. By Theorem~\ref{theo:informedl0consist}, we have $\lim_{n \to \infty} P(\hat{S}^{A,ei} = S) = 1$ in step 2 of Tran-s-ell0.
\end{document}