...
\end{equation}
\end{thm}
\paul{I replaced the factor in $b$ before the sum by a constant to simplify the expression. Dependence on $b$ beig fixed is already apprearant with the sum going from 1 to $b$}
...
\subsection{Data analysis with externally-informed penalization}\label{sec:informed_l0}

\paul{I changed this part and kept only $\hat S^{A,ei}$ because I realised the comparison of the properties of $\hat S^{EB,ei}$ compared to those of $\hat S^{EB,sd}$ was more subtle than I thought and I constructed an example where $\hat S^{EB,sd}$ greatly outperforms $\hat S^{EB,ei}$ in terms of probability of recovery, but $\hat S^{A,sd}$ does not outperform $\hat S^{A,ei}$ too much. Keeping only one also saves space.}

We propose a data analysis methods to set block penalties that does not require oracle values. The proposed method adapts to the relevance of the external-information which, as discussed in XXX, is driven by varying sparsity levels across blocks. Exploiting the connection between block $\ell_0$ penalties and Bayesian variable selection, we use an empirical Bayes approach to estimate the proportion of truly active variables in each block and set penalties. Relying on such connection also allows the use of fast Bayesian computational methods that overcome the intractability of $\ell_0$ penalties. Despite their Bayesian motivation, the methods is fully data-dependent and does not require any prior distribution.
...
the consistency of $\hat{S}^{A,ei}$ under betamin assumption,
...
$\lim_{n \to \infty} P(\hat{S}^{A,ei} = S) = 1$, and there exists a constant $c>0$ such that for all sufficiently large $n$ and any $\delta \in (0,1)$,
\begin{equation}\label{eq:rateconvregEB}
 P(\hat{S}^{A,ei} \neq S)\;\leq\; c\,\sum_{j=1}^b\,\, e^{-\tfrac{\delta}{2}\big[\kappa^{A}_j-\ln(p_j-s_j)\big]} + 
...
Assume now that sufficient assumptions for both $\hat S^{A,ei}$ and $\hat S^{A,sd}$ to be consistent are met and denote their convergence rate in \eqref{XXX}, $CR^{A,ei}$ and $CR^{A,sd}$ respectively. Assumme without loss of generality that the smallest signal is in block $b$, $\beta^*_{\min,b}=\beta^*_{\min}$. In a small signal setting such that 
...
and then
$$\frac{CR^{A,ei}}{CR^{A,ds}} = e^{-\frac{\delta}{2}((\beta^*_{\min}-\sqrt{\kappa^{A}_b})^2-(\beta^*_{\min}-\sqrt{\kappa^{A}})^2 + \ln(s/s_b))},$$
and $CR^{A,ei}<CR^{A,ds}$ since $\kappa^{A}>\kappa^{A}_b$ and $s>s_b$. The less sparse block $b$ is, the smaller $\kappa^{A}_b$ is compared to $\kappa^{A}$, the smaller $CR^{A,ei}$ is compared to $CR^{A,ds}$. 

In a large signal setting such that 
$$\kappa^{A}_b-\ln(p_b-s_b)=\min_{j=1,\ldots,b}\big\{(\beta^*_{\min,j}-\sqrt{\kappa^{A}_j})^2-\ln(s_j),\kappa^{A}_j-\ln(p_j-s_j)\big\} \quad \text{and}\quad 
\kappa^{A}-\ln(p-s)=\min\big\{(\beta^*_{\min}-\sqrt{\kappa^{A}})^2-\ln(s),\kappa^{A}-\ln(p-s)\big\},$$
we have
$$\frac{CR^{A,ei}}{CR^{A,ds}} = e^{-\frac{\delta}{2}(\kappa^{A}_b-\kappa^{A}) + \ln((p-s)/(p_b-s_b))},$$
and $CR^{A,ei}>CR^{A,ds}$.

That is, $\hat S^{A,ei}$ recovers most of the benefits of oracle block penalties. It has milder sufficient conditions for variable selection consistency than its non-informed counterpart $\hat S^{A,sd}$. It also has faster convergence rates in small signal settings by setting lower penalties in the blocks that are less sparse. The more informative is the block partition, that is one or several blocks being significantly less sparse than the global set of variables, the greater the gains. However, in large signals settings, its convergence rate is slower than that of $\hat S^{A,sd}$. This is because the strength of the smallest signal cannot be reliably estimated and, unlike the oracle penalties $\kappa^*_j$, the $\kappa^A_j$ do not grow with the $\beta_{\min,j}^*$. We show in our simulation study that this loss in probability of recovery that $\hat S^{A,ei}$ suffers in high signals settings is small when it is compared to $\hat S^{A,sd}$.

\section{Transfer learning for variable selection}
In this section we use the results of Section~\ref{sec:properties_legression} to obtain the properties of the transfer learning \hyperlink{lab:AlgoTL}{Algorithm 1}. This algorithm transfers the sets of variable selected in the source datasets, $\hat{S}^{(k)}$ $k=1,\ldots,K$, to ease variable selection in the target dataset. The set of variables in the target dataset $V$ is partitioned in two blocks, $B_1= \cup_{k=1}^K \hat S^{(k)}$ and $B_2=V\setminus  \cup_{k=1}^K \hat S^{(k)}$ and an externally-informed selector to select variables in the target dataset. In particuler, we derive the properties \hyperlink{lab:AlgoTL}{Algorithm 1} where the externally-informed selector is taken to be $\hat S^{A,ei}$. For simplicity we assumd the set of variables to be same in the target and source datasets. This assumption can be relaxed in our results at the expense of more complicated expressions.

Consider the

The properties of \hyperlink{lab:AlgoTL}{Algorithm 1} with externally-informed selector $\hat S^{A,ei}$ follow from Theorem~\ref{}. Consider the betamin assumption
\hypertarget{lab:A10}{(A10)}\quad For each block $j$, there exists $d_j\to \infty$ such that for  all sufficiently large $n$,
\begin{equation*}
        \sqrt{\frac{ (1-\xi)n\rho(\bX)}{6}}\beta_{\min,1}^* \,- \,\sqrt{\ln\big(|\cup_{k=1}^K(S^{(k)}\setminus S_1^L(\kappa^{\circ}))|\big)+\frac12\ln(n)} \;=\; \sqrt{\ln\big(|\cup_{k=1}^K(\hat S^{(k)}\cap S)|\big)} + d_1,\text{ and}\\
        \sqrt{\frac{ (1-\xi)n\rho(\bX)}{6}}\beta_{\min,2}^* \,- \,\sqrt{\ln\big(\big|\cap_{k=1}^K\big( \hat S^{(k)}\cup S_2^L(\kappa^{\circ}))^C|\big)+\frac12\ln(n)} \;=\; \sqrt{\ln\big(|\cap_{k=1}^K( S\setminus \hat S^{(k)})|\big)} + d_2,\text{ and}\\ .
    \end{equation*}
where $\xi=\tfrac12(1+\max_j \ln(p_j-s_j)/(\ln(p_j-s_j)+0.5\ln(n)))$.


By defintion of $B_1$ and $B_2$, we have
\begin{eqnarray}
    & s_1 = |B_1 \cap S| = |\cup_{k=1}^K(\hat S^{(k)}\cap S)|,\\
    & s_2 = |B_2 \cap S| = |\cap_{k=1}^K( S\setminus \hat S^{(k)})|,\\
    & p_1-|S_1^L(\kappa^{\circ})| = |B_1 \setminus S_1^L(\kappa^{\circ})| = |\cup_{k=1}^K(\hat S^{(k)}\setminus S_1^L(\kappa^{\circ}))|,\\
    & p_2-|S_2^L(\kappa^{\circ})| = |B_2 \cap {S_2^L(\kappa^{\circ})}^C| = \big|\cap_{k=1}^K\big( \hat S^{(k)}\cup S_2^L(\kappa^{\circ}))^C|.\\
\end{eqnarray}
