...
\hat{S}= \arg\max_{M\in \M} \left\{\max_{\beta\in \mathcal{L}_M}\ell(\by; \bbeta)\;-\;\kappa |M|\right\},
\end{equation}
...
\section{Properties of externally-informed penalization}
...
% In Section~\ref{ssec:regression_gains} we discuss how $\underline{\lambda}$ describes settings where $\hat S$ is not consistent but $\hat{S}^{ei}$ is.
...
\noindent Assumptions~\hyperlink{lab:A4}{A4}--\hyperlink{lab:A5}{A5} give ranges of penalties that are sufficient (but not necessary) for asymptotic support recovery. For simplicity, the discussion is restricted to choices of penalty $\kappa_j$ such that, in Assumption~\hyperlink{lab:A4}{A4}, $\ln(p_j-s_j)=O(f_j)$ for all $j$, so that $\gamma$ is bounded in $(0,1/2)$. For the standard selector $\hat{S}$, the single penalty $\kappa$ is essentially required to satisfy, for some sequences $f,g$ growing at arbitrarily slow rates, and up to constants
...
For a block selector $\hat{S}^{ei}$, the ranges for the $\kappa_j$'s essentially are, for some sequences $f_j,g_j$ growing at arbitrarily slow rates and up to constants
    ...
    \end{eqnarray}
The ranges in \eqref{eq:rangeblockpenex} imply that $\hat{S}^{i}$ requires milder conditions for selection consistency than $\hat{S}$. Intuitively, if there exist two blocks such that the ranges in \eqref{eq:rangeblockpenex} do not overlap, then a global penalty $\kappa$ cannot possibly satisfy \eqref{eq:rangeblockpenex} for all $j$ and consistent selection is essentially not possible. For example, this occurs if the global smallest active signal $\beta_{\min}^*$ is in block $b$ and satisfies  $\beta_{\min}^* - \sqrt{(n\rho(\bX))^{-1} \ln(s_b)} < \sqrt{\ln(p_1-s_1)}$.
...
%by definition, the ranges for $\hat{S}^{ei}$ are wider than that for $\hat S$. 
% That is, sufficient assumptions for variable selection consistency are milder with block penalties. 
Although not discussed here for brevity, block penalties also lead to similar improvements on the necessary assumptions, relative to those for standard $\ell_0$ penalties. Corollary~\ref{cor:selconstimpossiblereg} gives conditions under which consistent selection is possible with $\hat{S}^{ei}$ but not with $\hat S$.
...
% Let $\underline{\beta}^{*,b}_{\min,reg}$ and $\underline{\beta}^{*}_{\min,reg}$ be the smallest signal recoverable by $\hat{S}^{ei}$ and $\hat{S}$ respectively.
...
% Let ${OR}^b_{reg}$ be the oracle convergence rate for $\hat{S}^{ei}$ in Theorem~\ref{prop:fullrecovreg}, and ${OR}_{reg}$ that for $\hat{S}$. Then it holds that
% \begin{equation*}
...
Figures~\ref{fig:boundthresh} and~\ref{fig:boundthresh2} plot the ranges of penalty values $\kappa$ and $(\kappa_1,\kappa_2)$ ensuring selection consistency with $\hat{S}$ and $\hat{S}^{b}$ given in \eqref{eq:rangethreshex} and \eqref{eq:rangeblockthreshex} in the four examples, assuming $\beta^*_{\min,1}=2/3$, $\beta^*_{\min,2}=\beta^*_{\min}=1/10$, $f=g=\ln(ln(n))$ and $f_j=g_j=\ln(ln(n))$ for all $j$. In Example 2, asymptotic recovery with $\hat{S}$ is not possible in the range of values of $n$ considered while it is with $\hat{S}^{b}$.
In the other examples, asymptotic recovery with $\hat{S}$ is possible, but it requires larger $n$ than with $\hat{S}^b$. Example 1 shows that the gain can be large even when blocks are moderately discriminative, and Example 3 when some blocks are non-sparse.
...
We propose two data analysis methods to set block penalties that do not require oracle values. The proposed methods adapt to the relevance of the external-information which, as discussed in XXX, is driven by varying sparsity levels across blocks. Exploiting the connection between block $\ell_0$ penalties and Bayesian variable selection, we use an empirical Bayes approach to estimate the proportion of truly active variables in each block and set penalties. Relying on such connection also allows the use of fast Bayesian computational methods that overcome the intractability of $\ell_0$ penalties. Despite their Bayesian motivation, the methods are fully data-dependent and do not require any prior distribution.

Recall, by the BIC approximation in \eqref{eq:block_modelprob}, $NC(M)$ can be understood as the pseudo-posterior probability for model $M$ in a Bayesian variable selection framework where the $\kappa_j$'s are a suitable function of the block prior inclusion probability $\theta_j$. In such Bayesian framework, the posterior inclusion probability for a variable $j$ is given by the sum of the posterior probabilities of model including variable $j$. We then estimate the number of active variables in block $j$ by 
\begin{equation}\label{eq:shat}
    \hat{s}_j \,:=\, \sum_{i\in B_j}\,\,\sum_{M\in \M | i \in M}NC(M)
\end{equation}
for a given set block penalties $\kappa_1,\ldots,\kappa_b$.
The estimator of the proportion of active variables $\hat{s}_j/p_j$ can then be seen as an approximation to the empirical Bayes estimator of block prior inclusion probability $\theta_j$, as we discuss in Supplement XXX.
...
the consistency of $\hat{S}^{EB,ei}$ under
assumptions:
...
the consistency of $\hat{S}^{A,ei}$. It no longer requires Assumption~\hyperlink{lab:A7}{A7}, and Assumption~\hyperlink{lab:A8}{A8} is replaced by the more stringent Assumption A9: \\[5pt]
...
Our proposed selectors $\hat{S}^{EB,ei}$ and $\hat{S}^{A,ei}$ have similar but slightly stricter sufficient betamin conditions than the oracle selector with penalties $\kappa_j^*$. Assumption~\hyperlink{lab:A8}{A8} and~\hyperlink{lab:A9}{A9} ressemble the betamin assumption in \eqref{eq:suffoptim} with $\ln(p_j-s_j)$ replaced by, respectively $\ln(p_j/|S^{L}_j(\kappa^{\circ})|-1)+\frac12\ln(n)$ and $\ln(p_j-|S^{L}_j(\kappa^{\circ})|)+\frac12\ln(n)$. By definition, $\hat{S}^{EB,ei}$ and $\hat{S}^{A,ei}$ have slower convergence rates than the oracle selector. They approximate better the oracle selector when signals are small. Indeed, when $\sqrt{n\rho(\bX)/6}\beta_{\min,j}^*\approx \ln(p_j/s_j-1)$, the oracle penalty $\kappa_j^*$ in \eqref{eq:oraclepenreg} satisfies $\kappa_j^*\approx \ln(p_j/s_j-1)$ and $\kappa_j^{EB}$ and $\kappa_j^{A}$ are larger but not too far from the oracle block penalties. When $\sqrt{n\rho(\bX)/6}\beta_{\min,j}^*\gg \ln(p_j/s_j-1)$, we have $\kappa_j^*\approx \sqrt{n\rho(\bX)/24}\beta_{\min,j}^*$ and then,  $\kappa_j^*\gg \kappa_j^{EB}$ and $\kappa_j^*\gg \kappa_j^{A}$.

We compare next $\hat{S}^{EB,ei}$ and $\hat{S}^{A,ei}$ to fairer competitors, their respective non-informed counterparts. That is, selector $\hat{S}^{EB}$ that sets in Step 2 a single common penalty $\kappa^{EB}=\ln(p/\hat{s} -1)+ \tfrac{1}{2}\ln(n)$ and selector $\hat{S}^{A}$ that sets in Step 2 a single common penalty $\kappa^{A}=\ln(p-\hat{s})+ \tfrac{1}{2}\ln(n)$.

By Theorem~\ref{theo:empbayesselconsist}, $\hat{S}^{EB}$ is variable selection consistent under the betamin assumption
...
By Theorem~\ref{theo:informedl0consist}, $\hat{S}^{A}$ is variable selection consistent under the betamin assumption
\begin{equation}\label{eq:betaminEB}
    ...
    \end{equation}
where $\xi'=\tfrac12(1+\ln(p-s)/(\ln(p-s)+0.5\ln(n)))$ and $d\to \infty$. We have that for any $j$ and $n$ large enough,
...
In this thesis we establish the necessity of assumption~\eqref{eq:betaminnecreg} which, when applied to $\hat{S}$ ($\hat{S}^{ei}$ with $b=1$), implies
...
\begin{equation}\label{eq:CM}
    C(M)\;: =\;\tfrac{1}{2}\|\bX_M\tilde{\bbeta}^{(M)}\|^2-\sum_{j=1}^b\kappa_j |M_j|\quad\text{and}\quad NC(M)\;:=\;\frac{e^{C(M)}}{\underset{M' \in \M}{\sum} e^{C(M')}}.
\end{equation}
% \begin{lem}\label{lem:useful-reformulation} $\hat{S}^{ei}$ satisfies $\hat{S}^{ei}\;\in\;\arg\max_{M\in \M} NC(M)$
% \end{lem}
...
%$$L_{QM}\;:=\;\|\bX_{Q}\tilde{\bbeta}^{(Q)}\|^2-\|\bX_{M}\tilde{\bbeta}^{(M)}\|^2.$$
...
%That is, $\rho(\bX)$ measures how well one can approximate the $\boldsymbol{X}_{S \setminus M}$ with $\boldsymbol{X}_M$.
...
%\paul{I had to reprove this one because it was with factor 2 instead of $c$.}
...
%\paul{I had to reprove this one too because it was with factor 2 instead of $c$.}
...
Since $\hat{S}$ is $\hat{S}^{ei}$ with $b=1$, the assumptions of Lemma~\ref{lem:necbetaminblockthresh} for $\hat{S}$ are met and $\lim_{n \to \infty} P(\hat{S} = S) < 1$. Since Assumptions~\hyperlink{lab:A4}{A4} and \hyperlink{lab:A5}{A5} hold, by Propositions~\ref{prop:suffnecblockthreshI} (i) and \ref{prop:suffnecblockthreshII} (i), $\lim_{n \to \infty} P(\hat{S}^{ei} \subseteq S)= 1$ and $\lim_{n \to \infty} P(\hat{S}^{ei} \supseteq S)= 1$, and then $\lim_{n \to \infty} P(\hat{S}^{ei} = S)= 1$.\color{black}
...
\subsection{Proof of Corollary~\ref{cor:sfin}} Observe that the conditions of Lemma~\ref{lem:necbetaminblockthresh2} hold for $\hat{S}$ ($\hat{S}^{ei}$ for $b=1$), and then $\lim_{n\to \infty}P(\hat{S} = S)<1$. Since Assumptions~\hyperlink{lab:A4}{A4} and \hyperlink{lab:A5bis}{A5bis} hold, by Proposition~\ref{prop:suffnecblockthreshI} (i) and Proposition~\ref{prop:suffnecIIblockthreshbis}, $\lim_{n\to \infty}P(\hat{S}^{ei} \subseteq 1)= 1$ and $\lim_{n\to \infty}P( \hat{S}^{ei} \supseteq S)=1$, and then $\lim_{n\to \infty}P(\hat{S}^{ei} = S)=1$.
...
Since $\hat{S}$ is $\hat{S}^{ei}$ with $b=1$, the assumptions of Corollary~\ref{cor:necrecovreg} for $\hat{S}$ are met and $\lim_{n \to \infty} P(\hat{S} = S) < 1$. Since Assumptions~\hyperlink{lab:A4}{A4} and \hyperlink{lab:A5}{A5} hold, by Theorem~\ref{theo:suffcondlinearmodel} we have that $\lim_{n \to \infty} P(\hat{S}^{ei} = S)= 1$.
...
Since $\hat{S}$ is $\hat{S}^{ei}$ with $b=1$, the assumptions of Corollary~\ref{cor:necrecovregnonlin} for $\hat{S}$ are met and $\lim_{n \to \infty} P(\hat{S} = S) < 1$. Since Assumptions~\hyperlink{lab:A4bis}{A4bis} and \hyperlink{lab:A5bis}{A5bis} hold, by Theorem~\ref{theo:suffcondlinearmodelnonlinpen} we have that $\lim_{n \to \infty} P(\hat{S}^{ei} = S)= 1$.
