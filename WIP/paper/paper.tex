...
The rest of the paper is organized as follows. In Section~\{sec:intro_blockpen} we formally introduce externally-informed $\ell_0$ penalties and further detail their connection to Bayesian variable selection. As a particular case, we introduce our transfer learning algorithm for the transfer of selected variables. We also describe the general assumptions used across results. Section~\ref{sec:properties_legression} studies the properties of external information-dependent, block-based, $\ell_0$ penalties and how they ease consistent model recovery. We show that an oracle, knowing the true pattern of sparsity and signal strength, may take advantage of the external information or the transferred set of selected variables so that variable selection consistency is either attained where otherwise it would not be possible, or is attained at a faster rate. These oracle results give an understanding of the maximum gains achievable by leveraging external information. Results are tight, when applied to standard $\ell_0$ penalties with no access to external information, they match the minimax results of \cite{Bunea2007} under orthogonal design and match or improve the results on the fundamental limits on variable selection \cite{infotheowainwright, wang2010, bryon}. 

In Section XXX, we propose two-step data-based procedure grounded in empirical Bayes methods to set externally-informed $\ell_0$ penalties. We show the proposed procedure improves variable selection properties without requiring an oracle and compare its properties to those of oracle-set penalties. As a particular case, we discuss the properties of a concrete, non-oracle, transfer learning algorithm for the transfer of selected variables. In particular, we show it is robust to so-called \textit{negative transfer}, when transfer learning worsens the performance in the target. To the best of our knowledge, this is the first theoretical analysis of the a transfer learning algorithm centered on the transfer of the binary information on selection vs. no selection in source, rather than parameter estimates. This is relevant firstly because the transfer of selection information might be preferred when the similarity between source and target data is thought to lie in the sets of variables associated with the respective outcomes, rather than in the distance between respective parameter vectors. Secondly, transferring binary information on selection can be seen as a robust alternative to transferring parameter estimates when the \textit{domains}, the variables and their distributions, differ between the source and the target. Thirdly, our work highlights the value of transferring the set of selected variables independently of parameter estimates. Our transfer learning algorithm is also highly flexible, as it does not require full access to the source data, nor that the source task is a linear regression, nor that the set of variables is the same between the source and the target data.
...
Consider now a transfer learning setting where in each source dataset $k=1,\ldots,K$ the set of variables $\hat{S}^{(k)}$ was selected. The simplistic approach of forcing the inclusion in the target model of the variables selected in the sources datasets is highly prone to negative transfers as sets of variables associated to target and source outcomes may be similar but not identical. Given the connection between $\ell_0$ penalties and prior inclusion probabilities $\eqref{eq:block_modelprob}$, one may instead let a $\ell_0$ penalty differ between variables that were selected in the source datasets and those that were not.

More formally, one can form a vector of external information on the variables in the target linear model $\bz$ such that, for each $i=1,\ldots,p$, $z_i=1$ if $i\in \Cup_{k=1}^K\hat{S}^{(k)}$ and $z_i=0$ otherwise. The external information $\bz$ then induces a partition in two blocks, a first block that gathering variables that were selected at least once in the source datasets, and a second block gathering the rest of the variables. Given said partition, the externally-informed $\hat{S}^{ei}$ may be used to perform selection in the target.
...
The theoretical results of this thesis discuss variable selection properties of informed $\ell_0$ penalties when $\bZ$, possibly originating from a transfer of selected variables, induces a partition of variables as in \eqref{eq:partition} and when the number of samples and the number of variables go to infinity. Although not explicitly denoted, $s_j$, $p_j$ and $\beta_{\min,j}^*$ are to be thought of as functions of $n$, and so are $s$, $p$ and $\beta_{\min}^*$. I study the asymptotic regimes where \\[5pt]
...
Assumption~\hyperlink{lab:A1}{A1} can be slightly relaxed in my results to allow $b$ to grow slowly with $p$, but $b$ is assumed constant for simplicity. Similarly, Assumption~\hyperlink{lab:A2}{A2} is a mild assumption that can be relaxed but simplifies the exposition. Results are provided for both cases where the $s_j\geq1$ are fixed and $s_j \rightarrow \infty$. No assumption is made about the asymptotic regime linking $n$ and $p$, but the main interest is in $n= o(p)$ settings. Assumptions~\hyperlink{lab:A1}{A1}--\hyperlink{lab:A2}{A2} describe the general framework, in each result the needed assumptions are precisely specified.

The assumption of partition-inducing external information as in \eqref{eq:partition} made in Chapters~\ref{sec:properties_orth} to~\ref{sec:informed_l0} reflects real-world settings. One example is transfer learning where variables are partitioned between those selected in the source datasets and the rest. Another example arises when regressing on multimodal data or multiple types of variables (e.g., multi-omics) where meta-information suggests varying sparsity or signal strength (e.g., clinical history vs. genomic markers): the different modes or types naturally induce a partition. In Chapter~\ref{sec:empBayescomp}, I discuss more generally empirical Bayes methods to incorporate external information in variable selection, relaxing the assumption of partition-inducing external information.

Finally, the selector $\hat S^{ei}$ in \ref{eq:Mhat} assumes an externally-informed $\ell_0$ penalty that grows linearly in the model size. This linearity assumption not only allows connections to Bayesian spike-and-slab regression but also with frequentist selection with $\ell_1$ penalties under orthogonal design. While nonlinear $\ell_0$ penalties have been shown to be optimal for estimation and prediction \citep{Wu2013, Bunea2007}, I show that my results match or improve known limits on variable selection \citep{butucea2018,infotheowainwright}. In Section~\ref{sec:linregnonlinear} of Chapter~\ref{sec:properties_legression}, I also explore support recovery guarantees and rates of convergence for nonlinear, externally informed, block penalties.
